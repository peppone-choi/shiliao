...
```bash
git clone <repo> ~/.claude/skills/historical-sources   # the repo IS the skill
```

Point `SHILIAO_HOME` elsewhere only if you keep a corpus outside the repo — otherwise the
bundled `corpus/` is used automatically.

To rebuild from scratch (adding a source, or checking reproducibility):

```bash
SHILIAO_HOME=/tmp/c python3 -m shiliao.fetch   # ~20 min, 1,275 files, 44MB of text
SHILIAO_HOME=/tmp/c python3 -m shiliao.index   # ~1 min → index.db (85MB)
```
